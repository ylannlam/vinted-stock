...
export default function LotCard({ items, lotIndex, onMarkSent }) {
    ...
    <div className="col-span-full bg-white rounded-xl border border-orange-200 overflow-hidden shadow-sm">
      ...
      <div className="flex items-center gap-2 px-3 py-2 bg-orange-50">
        ...
        <div className="flex items-center gap-1.5 ml-auto">
          ...
          <button
            onClick={() => setOpen(v => !v)}
            className="text-[10px] font-semibold text-gray-500 bg-white border border-gray-200 px-2 py-0.5 rounded-full hover:bg-gray-50 transition-colors"
          >
            {open ? 'Réduire' : 'Voir'}
          </button>
          ...
          {!confirm ? (
            <button
              ...
              className="flex items-center gap-1 text-[10px] font-bold bg-orange-500 text-white px-2 py-0.5 rounded-full hover:bg-orange-600 transition-colors"
            >
              <svg className="w-2.5 h-2.5 pointer-events-none" fill="none" viewBox="0 0 24 24" stroke="currentColor">
